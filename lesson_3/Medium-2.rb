...
# Because the local variable an_array_param still points to the original object, the local variables my_array and an_array_param are still pointing at the same object
...
# This is assigning the output of the not_so_tricky_method function to new local variables (different objects)
...
# Ruby will automatically evaluate statements, so you can do this:
def color_valid(color)
