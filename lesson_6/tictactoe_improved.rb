...
def computer_places_piece!(brd)
  ...
  # 2. Then Defense if square is nil (no squares to offend)
  if !square
    WINNING_LINES.each do |line|
  ...
  # 3. If "square" is still nil after offense and defense...i.e there's no at risk square to defend and there's no square to offend...then computer randomly picks a square
  if !square
...
def place_piece!(brd, current_player)
  """
  Knows how to place the piece depending on 'current player'
  It calls player_places_piece! or computer_places_piece! depending on the value of current_player
  (You have to keep track of a current_player, and to switch that variable's value after every turn.)
  """

  # if current_player is the player, then the 'player_places_piece!' method runs. Else, the computer places a piece.
  current_player == 'player' ? player_places_piece!(brd) : computer_places_piece!(brd)
end

def alternate_player(current_player)
  """Alternate the current player based on the current player"""
  
  # returns 'computer' if current_player is 'player' and vice versa
